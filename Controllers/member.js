...
exports.getAllMember = async(req,res)=>{
    try{
        ...
        const totalMember = members.length;
         ...
         res.status(200).json({
            message:members.length?"Fetched Members SuccessFully":"No Such Member Registered yet",
    ...
    try{
        ...
        const member = await Member.find({gym:req.gym._id,
            nextBillDate:{
                ...
                $lte:nextThreeDays  // less thna or equal to 3 days from today
            }
        });
        res.status(200).json({
...
exports.expiringWithIn4To7Days = async(req,res)=>{
    try{
        ...
        next4Days.setDate(today.getDate()+7);
        
        const member = await Member.find({gym:req.gym._id,
            nextBillDate:{
                $gte:next4Days,  // greater than or equal to 4 todays later from today
                $lte:next7Days  // less thna or equal to 7 days from today
            }
        });
        res.status(200).json({
